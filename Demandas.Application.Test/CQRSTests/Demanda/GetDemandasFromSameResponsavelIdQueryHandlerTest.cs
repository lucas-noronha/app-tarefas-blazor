...
{
    public class GetDemandasFromSameResponsavelIdQueryHandlerTest
    {
        private readonly Mock<IDemandaService> _mockDemandaService;

        public GetDemandasFromSameResponsavelIdQueryHandlerTest()
        {
        ...
        public async Task Handle_ShouldReturnDemandasFromSameClienteId()
        {
            ...
            var handler = new GetDemandasFromSameResponsavelIdQueryHandler(_mockDemandaService.Object);

            // Act
            var result = await handler.Handle(new GetDemandasFromSameResponsavelIdQuery(1), new CancellationToken());
